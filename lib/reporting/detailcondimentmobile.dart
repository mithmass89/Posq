...
class DetailCondimentTerjualMobile extends StatefulWidget {
  ...
  @override
  State<DetailCondimentTerjualMobile> createState() =>
      _DetailCondimentTerjualMobileState();
}

class _DetailCondimentTerjualMobileState
    extends State<DetailCondimentTerjualMobile> {
  ...
  Future<List<dynamic>> olahData() async {
    for (var x in widget.listoutlets) {
      await ClassApi.CondimentDetail(
        widget.fromdate,
        widget.todate,
        x,
      ).then((value) {
    ...
    groupedData.forEach((key, value) {
      ...
      int sumNetRevenue = value.fold(0, (previousValue, item) {
        return previousValue + (item['totalnett'] as int);
      });
                              ...
                              ? ListTile(
                                  onTap: () async {
                                    await Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                          builder: (context) =>
                                              DetailCondimentCheck(
                                                fromdate: widget.fromdate,
                                                todate: widget.todate,
                                                optioncode: data[index]
                                                    ['itemdesc'],
                                              )),
                                    );
                                  },
